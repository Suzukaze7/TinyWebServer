...
namespace suzukaze {
template <typename Func = std::packaged_task<void()>> class ThreadPool {
public:
    ThreadPool(std::size_t thread_count = std::thread::hardware_concurrency(), std::size_t max_queue_size = 1000);
...
template <typename Func> inline ThreadPool<Func>::~ThreadPool() {
    done = true;
    consume_cv.notify_all();
}

template <typename Func> inline void ThreadPool<Func>::work() {
    while (!done) {
...
template <typename Func> inline void ThreadPool<Func>::submit(Func task) {
    std::unique_lock guard(lock);
